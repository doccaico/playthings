...
func getCurrentVersion(path string) (string, error) {
	...
	res = strings.Replace(res, "windows/amd64", "", -1)
	res = strings.TrimSpace(res)
	return res, nil
}

func getNewVersion(path string) string {
	// get: "go1.22.2.windows-amd64.zip"
	// return: "go1.22.2"
	return strings.Replace(path, ".windows-amd64.zip", "", -1)
}

func main() {

	seven_zip_path := isExist("7za")
	if seven_zip_path == "" {
		log.Fatal("7za.exe is not found")
	}
	...
	output_dir := fmt.Sprintf("-o%s", filepath.Join(userprofile, "Downloads"))

	fmt.Println("[Extraction] ...")
	cmd := exec.Command(seven_zip_path, "x", "-aoa", abs_path, output_dir)
	if err := cmd.Run(); err != nil {
		log.Fatal(err)
	}

	source := filepath.Join(userprofile, "Downloads", "go")
	dest := filepath.Join(os.Getenv("SystemDrive"), string(os.PathSeparator), "go")
